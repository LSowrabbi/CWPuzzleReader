            ...
            for j in range(0,width):
                ...
                if j<width-1 and (cellblock[i][j+1]!="." and cellblock[i][j+1]!=":"):
                    if (cellblock[i][j]!="." and cellblock[i][j]!=":") and (j==0 or (cellblock[i][j-1]=="." or cellblock[i][j-1]==":")):
                        ...
                        across[acc].append(findcurracross(i,j))
                        acc=acc+1
                        num=num+1
                        count=count+1
                if i<height-1 and (cellblock[i+1][j]!="." and cellblock[i+1][j]!=":")  :       
                    if (cellblock[i][j]!="." and cellblock[i][j]!=":") and (i==0 or (cellblock[i-1][j]=="." or cellblock[i-1][j]==":")):
                        ...
                        down[dwn].append(findcurrdown(i,j))
                        num=num+1
                        dwn=dwn+1
                j=i+1
            i=i+1
...
for i in range(0,height):
    ...
    while(j!=width):
        inter[i].append([])
        if(cellno[i][j]!=0 and (j==0 or cellblock[i][j-1] in [".",":"])):
           if(j+2<width and cellblock[i][j+1]!="." and cellblock[i][j+2] not in [".",":"]):
                temp=[]
                clue=cellno[i][j]
                char=0
                temp=["across",clue,char]
                inter[i][j].append(temp)
           else:
                clue=0
        else:
            if(cellblock[i][j] not in [".",":"] and clue!=0):
                char=char+1
                temp=["across",clue,char]
                inter[i][j].append(temp)
        j=j+1
...
for j in range(0,width):
    i=0
    while(i!=height):
        if(cellno[i][j]!=0 and (i==0 or cellblock[i-1][j] in [".",":"])):
           if(i+2<height and cellblock[i+1][j]!="." and cellblock[i+2][j] not in [".",":"]):
                temp=[]
                clue=cellno[i][j]
                char=0
                temp=["down",clue,char]
